...
# Optional: VPC Resource Controller Policy
...
resource "aws_iam_role" "airflow_role" {
  name = "${var.cluster_name}-airflow-role"
    ...
    Statement = [
      {
        ...
        Condition = {
          StringEquals = {
            "${replace(data.aws_eks_cluster.cluster.identity[0].oidc[0].issuer, "https://", "")}:sub": "system:serviceaccount:airflow:airflow-worker"
          }
        }
      }
    ]
